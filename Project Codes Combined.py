...
# %%
#Temporal Analysis
new_df = df.loc[:,['year','age','c_temp','snowfall','rainfall','disasters','storms','spending']]
...
geographical_analysis_city = df.groupby(['City'])[['rainfall', 'snowfall','c_temp','disasters','storms']].mean().reset_index()
...
plt.figure(figsize=(12, 6))
sns.lineplot(data=df, x='year', y='g_temp', label='Global Temperature')
sns.lineplot(data=df, x='year', y='happening', label='US Opinion of Climate Change')
plt.xlabel('Year')
plt.ylabel('Value')
...
plt.figure(figsize=(12, 6))
sns.scatterplot(data=df, x='rainfall', y='c_temp',hue = 'happening')
plt.xlabel('Rainfall')
